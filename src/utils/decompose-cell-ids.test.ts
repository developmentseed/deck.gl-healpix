...
describe('decomposeCellIds', () => {
  it('decomposes NEST cell IDs at nside=1', () => {
    ...
    expect(faceIx[0]).toBe(0 << 18); // face=0
    expect(faceIx[1]).toBe(1 << 18); // face=1
    expect(faceIx[2]).toBe(11 << 18); // face=11
    expect(iy[0]).toBe(0);
    expect(iy[1]).toBe(0);
    expect(iy[2]).toBe(0);
  });
  ...
  it('decomposes RING cell IDs at nside=4', () => {
    ...
    expect(faceIx[0]).toBe((0 << 18) | 3);
    expect(iy[0]).toBe(3);
    expect(faceIx[1]).toBe((1 << 18) | 3);
    expect(iy[1]).toBe(3);
    expect(faceIx[2]).toBe((1 << 18) | 2);
    expect(iy[2]).toBe(2);
  });
  ...
  it('packing matches healpix-ts decomposition for non-trivial cell', () => {
    ...
    const { faceIx, iy } = decomposeCellIds(new Uint32Array([cellId]), nside, 'nest');
    expect(faceIx[0]).toBe((f << 18) | x);
    expect(iy[0]).toBe(y);
  });
